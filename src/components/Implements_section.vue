<template>
  ...
  <section class="w-full">
    <!-- Título -->
    <div class="w-full text-left py-8 px-8">
      <h1 class="text-2xl font-bold">IMPLEMENTOS DEPORTIVOS</h1>
    </div>

    <!-- Tarjetas de productos -->
    <div class="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-6">
      <div
        v-for="(producto, index) in productos"
        :key="index"
        class="bg-white rounded-4xl shadow-2xl overflow-hidden p-2"
      >
        <img
          :src="producto.imagen"
          :alt="producto.nombre"
        ...
        <div class="p-4">
          ...
          <a
            :href="`https://wa.me/3183902084?text=Hola, estoy interesado en el producto ${producto.nombre}`"
            target="_blank"
